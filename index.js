...
const use_config = async (config) => {
  ...
  try {
    ...
    stream.on("data", function (data) {
      if (true) {
        ...
        data.messages.forEach((message) =>
          meta_parser(message, result, temp_stats)
        );
        data.messages.forEach((message) =>
          text_parser(message, result, temp_stats)
        );
        post_meta(result, temp_stats);
        let filename = data.name + "_" + data.id + ".json";
        fs.writeFile(
          path.join(__dirname, ...config_data.output, filename),
        ...
        fs.writeFile(
          path.join(__dirname, ...config_data.formatted_text, filename),
          beautify(result, config),
          (err) => {
            if (err) console.log(err);
    ...
    fs.createReadStream(path.join(__dirname, ...config_data.filepath)).pipe(
      stream
    );
  } catch (err) {
    throw err;
